export class Scene {
    ...
    initializePuzzles(puzzleManager) {
    ...
    update(deltaTime, era) {
        this.entities.forEach(entity => entity.update(deltaTime));
    }
    render(ctx, assetManager, era = 'present') {
        ctx.fillStyle = '#333';
        ...
        eraObjects.forEach(obj => {
            if (obj.puzzleId && this.puzzleManager) {
                ...
                if (obj.id === 'bridge' && !puzzleState.bridgeIntact)
                    return;
        ...
        eraInteractables.forEach(interactable => {
            if (interactable.puzzleId && this.puzzleManager && this.puzzleManager.isPuzzleSolved(interactable.puzzleId))
                return;
    ...
    handleInteraction(playerPos, era) {
        this.lastInteractionResult = null;
        if (this.puzzleManager) {
            const interactables = this.interactables[era] || [];
            for (const item of interactables) {
                if (playerPos.x < item.x + item.width &&
        ...
        for (const obj of objectives) {
            if (playerPos.x < obj.x + obj.width &&
